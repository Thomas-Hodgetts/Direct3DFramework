...
void Camera::Walk(float Direction)
{
	...
	XMVECTOR Pos = GetEye();
	XMStoreFloat3(&m_Eye, XMVectorMultiplyAdd(Speed, NewLook, Pos));
}
//Book
void Camera::Strafe(float Direction)
{
	...
	XMVECTOR Pos = GetEye();
	XMStoreFloat3(&m_Eye, XMVectorMultiplyAdd(Speed, Right, Pos));
}
//Book
void Camera::Pitch(float Angle)
{
	XMMATRIX Rotation = XMMatrixRotationAxis(GetRight(), Angle);
	XMStoreFloat3(&m_Up, XMVector3TransformNormal(GetUp(), Rotation));
	XMStoreFloat3(&m_At, XMVector3TransformNormal(GetAt(), Rotation));
}
//Book
void Camera::Yaw(float Angle)
{
	XMMATRIX Rotation = XMMatrixRotationY(Angle);
	XMStoreFloat3(&m_Right, XMVector3TransformNormal(GetRight(), Rotation));
	XMStoreFloat3(&m_Up, XMVector3TransformNormal(GetUp(), Rotation));
	XMStoreFloat3(&m_At, XMVector3TransformNormal(GetAt(), Rotation));
}
